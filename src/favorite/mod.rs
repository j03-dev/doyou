...
pub fn FavoriteCard(track: YoutubeTrack, on_remove: Callback<String>) -> Element {
    ...
    rsx! {
        div {
            class: "card bg-base-100 shadow-md hover:shadow-xl transition-all duration-300 cursor-pointer overflow-hidden",
            figure {
                class: "w-full h-40",
                img {
            ...
            div {
                class: "card-body bg-base-200 p-3",
                    div {
                        class: "flex items-start justify-between gap-2",
                        div {
                            class: "flex-1 min-w-0",
                            h2 {
                                class: "card-title text-sm truncate text-base-content",
                                "{track.title}"
                            }
                            p {
                        ...
                        div { class: "flex gap-1",
                            IconButton {
                                on_click: move |_| on_remove.call(track_id.clone()),
